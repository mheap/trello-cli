...
- 4738f6c: Add a `notification` command group for managing Trello notifications: `notification:list` (list your notifications), `notification:show` (show a single notification), `notification:read` and `notification:unread` (mark a notification read/unread), and `notification:read-all` (mark all as read).

## 1.5.0

### Minor Changes

- 373efb7: Add interactive terminal UI (`trello interactive`)
...
- Updated dependencies [da2e9d5]
  - @trello-cli/cache@1.0.5
