var __ = function(program, output, logger, config, trello, translator) {
	...
	trelloApiCommand.makeTrelloApiCall = function(options, onComplete) {
		...
		try {
			sourceListId = translator.getListIdByBoardNameAndListName(
				options.sourceBoard,
				options.sourceList
			);
			destinationBoardId = translator.getBoardIdByName(
				options.destinationBoard
			);
			destinationListId = translator.getListIdByBoardNameAndListName(
				options.destinationBoard,
			...
			logger.error(
				err.message +
					" (try refreshing the cache with 'refresh' command, or check spelling)."
			);
			if (!options.verbose) {
				process.exit(1);
			} else {
